...
Alle Grenzwerte sind an den ABGENOMMENEN Saetzen light-e und light-i kalibriert:
...
LIM = {"e": dict(px=(1260,1810), top=(1,4), bot=(74,2)),
       "f": dict(px=(1460,2110), top=(0,4), bot=(75,1)),
       "g": dict(px=(1430,2060), top=(0,4), bot=(75,1)),
       "i": dict(px=(1170,1680), top=(0,4), bot=(75,1))}
...
SIZE_SPAN  = 1.25   # groesstes/kleinstes Bild. Referenz: e 1.12, i 1.19
...
for k in sys.argv[1:]:
    lim=LIM[k]
    vo=[p[:3] for p in Image.open(f"src/assets/enemy-light-{k}.png").convert("RGBA").getdata() if p[3]>=128]
    vh, va = hist(vo), avg(vo)
    ms=[measure(f"src/assets/enemy-light-{k}-move-{i}.png") for i in range(1,13)]
    print(f"\n=== light-{k} ===")
    for i,m in enumerate(ms,1):
        dist=sum((a-b)**2 for a,b in zip(va,m["avg"]))**0.5
        hv=inter(vh,m["hist"])
        bad=[]
        if m["size"]!=(56,76): bad.append(f"Groesse {m['size']}")
