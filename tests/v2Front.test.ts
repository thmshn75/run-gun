...
describe('Run Gun V2 — S4 die beiden Flaechen', () => {
  ...
  it('ist bei 16 und 32 ms bildratenunabhaengig', () => {
    ...
    // Rate mal Schrittweite, hier 1,6 * 0,032 = rund 5 Prozent. Die Schranke von
    // 1 Prozent ist also fuenfmal strenger als die Theorie zulaesst und faengt
    // jeden echten Bildratenfehler, der sich in Faktoren statt Promille zeigt.
    // Auf dem Bildschirm sind 1 Prozent von 300 px drei Pixel.
    ...
    expect(relativ(in16.vorrat, in32.vorrat)).toBeLessThan(1e-2)
    expect(relativ(in16.eigenerWert, in32.eigenerWert)).toBeLessThan(1e-2)
    expect(relativ(in16.frontY, in32.frontY)).toBeLessThan(1e-2)
  })
