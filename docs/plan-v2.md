...
| **W7 — Plastische Figuren + zwei Gamefeel-Korrekturen** *(neu 2026-08-22, erweitert 2026-08-23, läuft VOR W6)* | **Teil 1 — Optik (Codex-Auftrag):** Den optischen Abstand zum 3D-Vorbild schließen, das Thomas als Referenz geschickt hat — mit den Mitteln eines 2D-Motors. Echtes 3D ist ausgeschlossen (Phaser ist ein 2D-Renderer, ein Wechsel wäre ein anderer Motor). Nach Wirkung sortiert: **(a)** beleuchtete statt flacher Sprites für Truppe, alle drei Gegnertypen und Boss — Volumen durch Licht von oben, Schattenseite, Kantenlicht; **(b)** glatte statt harter Pixelkanten: Sprites in doppelter Auflösung, `pixelArt: true` in `main.ts` fällt; **(c)** Bodenschatten sind bereits gebaut und bleiben unverändert. Alle vorhandenen Sprites stammen aus Codex-Läufen, das Verfahren „groß rendern, dann herunterrechnen" ist etabliert, Vorlagen liegen in `assets/probe/` (136x184). Claude schreibt die Spec und reviewt, Codex erzeugt die Bilder. **Teil 2 — zwei Korrekturen aus Thomas' iPhone-Test vom 2026-08-23 (Claude direkt, läuft VOR Teil 1, damit Codex auf dem fertigen Balance-Stand arbeitet):** **(d) Gegnermenge steigt stärker mit der Levelnummer** — W5 passt am iPhone, aber die Kurve über die Level ist zu flach („Gegnermenge darf mit Levels noch steigen"). Angefasst wird die Levelkurve, nicht der Startwert von Level 1 — der ist abgenommen. **(e) Der Boss pendelt nicht mehr seitlich**, sondern kommt nur noch langsam auf die Truppe zu; `boss.phaseOne/phaseTwo.moveSpeed` und `moveAcrossRoad` entfallen, `advanceSpeed` (8,35 px/s) bleibt unverändert — „Geschwindigkeit wie jetzt" ist als das bestehende Vorrück-Tempo gelesen, nicht als Übertragung der Pendelgeschwindigkeit (markierte Annahme, Lesson 2026-08-22 „Vergleichs-Wunsch in die falsche Richtung"). | **Teil 1:** Jede ersetzte Figur liegt in doppelter Auflösung vor und wird auf die gemessene Spielgröße herunterskaliert (kein Hochskalieren); die gemessenen Körpermaße in `balance.ts` (`bodyWidth`/`bodyHeight`) sind gegen die neuen Bilder nachgemessen, nicht übernommen; `pixelArt: false` verändert nachweislich keine Trefferflächen (Tests grün); ein Kontrollbild zeigt alle Figuren auf Fahrbahn **und** Umgebung — beide Hintergründe müssen tragen (Lesson aus dem Betongrau-Versuch); am iPhone wirken die Figuren plastisch statt flach (Thomas-Urteil, das Referenz-Reel ist der Vergleichsmaßstab). **(d):** Die Gegner-je-Sekunde-Kurve über Level 1–12 ist **gemessen** (Sonde gegen eine vom Spawner unabhängige Schranke, Lesson 2026-08-22 „Test schrieb die Formel ab"), Level 1 bleibt im abgenommenen Bereich, und der Level-12-Wert liegt nachweislich über dem heutigen; zusätzlich im Browser belegt, dass die Erhöhung **ankommt** und nicht an der Spurvergabe verpufft (`deferred`-Quote vor/nach, Lesson 2026-08-22 „Drei Anläufe an der Hordengröße"). **(e):** Per Test nachgewiesen, dass der Boss seine X-Position während des Kampfs nicht mehr ändert; die Boss-Kampfdauer ist nach der Änderung erneut gemessen (der stehende Boss bleibt dauerhaft in der Feuerlinie, die Dauer muss also sinken — Vorhersage vor der Messung, Lesson 2026-08-22 „Gegenmaßnahme half dem Gegner"); fällt sie aus dem Fenster 20–40 s, wird das als Befund gemeldet, nicht still nachbalanciert. |
